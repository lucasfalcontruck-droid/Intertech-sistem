...
/** app/(app)/pedidos/compra-insumo/page.tsx — Pedidos de compra do tipo INSUMO. */
...
export default function CompraInsumoPage() {
  const { data, isLoading, isError, error } = usePedidosCompra(FornecedorTipo.INSUMO);
  ...
  return (
    <div>
      <PageHeader
        title="Pedidos de compra — Insumo"
        subtitle="Compras de insumos junto a fornecedores"
        action={
      ...
      {data && (
        <Card className="p-0">
          {data.pedidos.length === 0 ? (
            <EmptyState message="Nenhum pedido de compra de insumo ainda." />
          ) : (
            <div className="overflow-x-auto p-5">
              <table className="w-full min-w-[820px] border-collapse">
                <thead>
                  <tr className="border-b border-border">
                    {["Pedido", "Fornecedor", "Insumo", "Quantidade", "Valor", "Status", "Previsão", ""].map(
                      (h) => (
                        <th
                          key={h}
